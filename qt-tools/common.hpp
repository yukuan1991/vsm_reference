...
inline nlohmann::json check_for_update (const char* server_addr, const char* path, std::experimental::string_view software_name) try
{
    auto data = ::json_http_get (server_addr, path, {{"software_name", software_name.to_string ()}});
    auto & files = data ["info"];

    auto update_files = nlohmann::json::array ();
    for (auto& file : files)
    {
