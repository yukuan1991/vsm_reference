...
inline std::vector<std::string> check_for_update (const char* server_addr, const char* path, std::experimental::string_view software_name) try
{
    ...
    for (auto& file : files)
    {
        ...
        {
            update_files.emplace_back (path);
        }
    }
